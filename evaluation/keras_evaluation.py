...
def keras_evaluation(args_from_script=None):
	...
	# inverted hierarchy
	input_file = "/net/scratch_cms3b/fackeldey/trainingssets/full_MSSM_HWW_trainingsset.root"

	# Open input file and register branches with input and output variables
	file_ = ROOT.TFile(input_file, "update")
	if file_ == None:
		raise Exception(
			"File is not existent: {}".format(input_file))

	tree = file_.Get(args.tree)
	if tree == None:
		raise Exception("Tree {} is not existent in file: {}".format(args.tree, input_file))
	...
	for i_event in range(tree.GetEntries()):
		...
		response = classifiers[event % 2].predict(values_stacked)
